...
Для того, чтобы в коде не использовать длинные тексты и кириллицу, название компетенций (шкал) были перекодированы в лейблы `c1`, `c2`, `c3` для высокоуровневых категорий (5 макрокомпетенций), шкал компетенций (8) и суб-шкал ()
...
Названия шкал и присвоенные им коды можно посмотреть в табличке:
...
competencies_data <- copy(questionnaire)
names(competencies_data) <- tolower(names(competencies_data))
names(competencies_data) <- gsub('[0-9]*\\.\\s+', '', names(competencies_data))

competencies_data <- competencies_data[, .SD, .SDcols = c2_questions]
...
```{r, include=FALSE}
competencies_data_l <- melt(competencies_data, measure.vars = names(competencies_data), variable.name = 'q')
